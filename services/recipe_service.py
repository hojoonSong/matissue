...
class RecipeService:
    ...
    async def search_recipes(self, value: str, skip: int = 0, limit: int = 160):
        try:
            recipes = await recipe_dao.search_recipes_with_comments(value, skip=skip, limit=limit)
            return recipes
        except Exception as e:
    ...
    async def get_recipes_by_single_serving(self, skip: int = 0, limit: int = 160):
        try:
            results = await self.recipe_dao.get_recipes_by_single_serving_with_comments(
                skip=skip, limit=limit
            )
            return results
        except Exception as e:
